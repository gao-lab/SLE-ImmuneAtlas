...
def do_harmony(adata, n_top_genes=2000, batch="sample", theta=2, resolution=0.8,
               hvg_batch=True, hvg_only=False, max_iter_harmony=20):
    r"""
    Run harmony on adata object
    """
    if hvg_batch:
    ...
    Z = harmonize(adata.obsm["X_pca"], adata.obs, batch_key=batch, max_iter_harmony=max_iter_harmony, theta=theta, use_gpu=GPU_flag)
