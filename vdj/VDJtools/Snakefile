...
rule all:
    input:
        # expand("./out/vdj_format/{sample}/bcr_vdjtools.tsv", sample = SAMPLES), # STEP1
        # "bcr_vdjtools.basicstats.txt",  # STEP2
        # expand("{sample}.fancyvj.wt.pdf", sample = SAMPLES), # STEP3
        # expand("{before}_{after}.aa.paired.scatter.pdf", zip, before=BEFORE, after=AFTER), # STEP4
        # pic = "pairwise_distances.intersect.batch.aa.pdf", # STEP5
        # file = "pairwise_distances.intersect.batch.aa.txt" # STEP5
        # pic1 = expand("cluster_sample.hc.aa.{method}.pdf", method= OVERLAP_METHOD),# STEP6
        # pic2 = expand("cluster_sample.mds.aa.{method}.pdf", method= OVERLAP_METHOD), # STEP6
...
rule convert_10x_to_VDJtools:
    input:
        raw_file = "./bcr_data/{sample}_bcr.csv"
    output:
        tmp_dir = directory("./out/vdj_format/{sample}"),
        vdj_file = "./out/vdj_format/{sample}/bcr_vdjtools.tsv"
    script:
...
rule vdjtools_basic_whole:
    ...
    params:
        prefix = "bcr_vdjtools"
    shell:
        # java -jar ./vdjtools/vdjtools-1.2.1.jar PlotFancyVJUsage  {input.vdj_file}  {wildcards.sample}_bcr
        """
...
rule compare_share:
    input:
        vdj_file_before =  "./out/vdj_format/{before}/bcr_vdjtools.tsv",
        vdj_file_after =  "./out/vdj_format/{after}/bcr_vdjtools.tsv",
    output:
        plot_pair = '{before}_{after}.aa.paired.scatter.pdf'
    shell:
        # before and after can not have same name(even in different dir)
        '''
         cp {input.vdj_file_before} ./out/vdj_format/{wildcards.before}/{wildcards.before}_bcr_vdjtools.tsv
         java -jar ./vdjtools/vdjtools-1.2.1.jar OverlapPair -p -i aa  ./out/vdj_format/{wildcards.before}/{wildcards.before}_bcr_vdjtools.tsv {input.vdj_file_after} {wildcards.before}_{wildcards.after}
         # rm {input.vdj_file_before}_tmp
         rm ./out/vdj_format/{wildcards.before}/{wildcards.before}_bcr_vdjtools.tsv
        '''
...
rule cluster_sample:
    ...
    shell:
        ...
        java -jar ./vdjtools/vdjtools-1.2.1.jar ClusterSamples -e {params.method} -i aa -p  -f treatment -l name   {params.prefix} 
        '''
