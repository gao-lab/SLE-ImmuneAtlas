...
DotPlot2(cd4_filter, marker_list = c('LRRN3','CCR7','NPM1','SELL','PASK','IL7R',
                                       ...
                                       'FOXP3'))
DotPlot2(cd4_filter, marker_list = c('CXCR3','CCL5','GZMK', 'PDCD1', 'TBX21', 'GATA3', 'RORC'))
...
cd4_filter$subtype[which(cd4_filter$seurat_clusters %in% c(9))] <- 'T.CD4.IFN-response'
cd4_filter$subtype[which(cd4_filter$seurat_clusters %in% c(5, 10))] <- 'T.CD4.Treg'
cd4_filter$subtype[which(cd4_filter$seurat_clusters %in% c(3))] <- 'T.CD4.Th1.cxcr3'
cd4_filter$subtype[which(cd4_filter$seurat_clusters %in% c(2))] <- 'T.CD4.Th1'
cd4_filter$subtype[which(cd4_filter$seurat_clusters %in% c(7))] <- 'T.CD4.Th2'
cd4_filter$subtype[which(cd4_filter$seurat_clusters %in% c(6))] <- 'T.CD4.Th17'
cd4_filter$subtype[which(cd4_filter$seurat_clusters %in% c(8))] <- 'T.CD4.Tfh'
cd4_filter$subtype[which(cd4_filter$seurat_clusters %in% c(0,1,4))] <- 'T.CD4.naive'
...
#---------------------------------- Save ---------------------------------------
select.cells <- CellSelector(plot = DimPlot(cd4_filter, label = T))
cd4_filter <-subset(cd4_filter,cells = select.cells, invert = T)

save(cd4_filter,file = './final/addtional/seurat/t_cell/04-CD4_Tcell_filter_anno.rdata')
