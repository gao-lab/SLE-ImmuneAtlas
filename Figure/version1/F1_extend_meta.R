...
# ---------------------------- pie plot of the group ---------------------------
pie_df <- extend_meta$group %>% table() %>% data.frame() 
pie_df$. <- str_replace(pie_df$.,pattern = '_',replacement = ' ')
pie_df$. <- factor(pie_df$., levels = c('hc child','hc','sle','sle child','sle flare','sle treated','IFNbeta stim'))
pie(x =pie_df$Freq ,labels = pie_df$Freq ,col = c('#377EB8','#4DAF4A','#A65628','#FF7F00','#984EA3','#E41A1C','#DA9494'),
                                      lty=0,main = 'group',clockwise = T,)

legend("right", x=1.2,y=0.7,
       inset=-0.2, #图例为关键词时，inset = 分数 设置其相对位置(-向下，+向上)
       legend=pie_df$. , #图例文字
       fill=c('#377EB8','#4DAF4A','#A65628','#FF7F00','#984EA3','#E41A1C','#DA9494'), #图例填充颜色
...
#----------------------  pie plot of the dataset -------------------------------
pie_df <- extend_meta$dataset %>% table() %>% data.frame() 
pie_df$. <- str_replace(pie_df$.,pattern = '_',replacement = ' ')
pie_df$. <- factor(pie_df$., levels = c('nbt 17','pnas 19','nc 19','ni 20','ebi 21','core dataset'))
# pie_df %<>% arrange(.)
pie(x =pie_df$Freq ,labels = pie_df$Freq ,col = c('#377EB8','#4DAF4A','#A65628','#DA9494','#984EA3','#E41A1C'),
    lty=0,main = 'group',clockwise = T,)

legend("right", x=1.2,y=0.7,
       inset=-0.2, #图例为关键词时，inset = 分数 设置其相对位置(-向下，+向上)
       legend=pie_df$. , #图例文字
       fill = c('#377EB8','#4DAF4A','#A65628','#DA9494','#984EA3','#E41A1C'), #图例填充颜色
